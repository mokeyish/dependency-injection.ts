...
import { getFactoryDef , FactoryFn } from '../definition';
import { throwCyclicDependencyError, throwInvalidProviderError, throwMixedMultiProviderError } from '../errors';
 ...
 */
let NULL_INJECTOR: Injector | undefined;
...
export class R3Injector implements Injector {
  ...
  constructor(
    ...
    additionalProviders &&
        deepForEach(
            additionalProviders,
            (provider: SingleProvider) => this.processProvider(provider, def, additionalProviders));

    deepForEach([def], (injectorDef: InjectorType<any> | InjectorTypeWithProviders<any>) => this.processInjectorType(injectorDef, [], dedupStack));
      ...
      if (!(flags & InjectFlags.SkipSelf)) {
        ...
        if (record === undefined) {
          ...
          if (def && this.injectableDefInScope(def)) {
            ...
            // all along.
            record = makeRecord<T>(injectableDefOrInjectorDefFactory(token), NOT_YET);
          } else {
            record = null;
          }
  ...
  private processInjectorType(
    ...
    if (def.imports != null && !isDuplicate) {
      ...
      try {
        deepForEach(def.imports, (imported: any) => {
    ...
    if (defProviders != null && !isDuplicate) {
      ...
      deepForEach(
          defProviders, (provider: SingleProvider) => this.processProvider(provider, injectorType, defProviders));
    }
